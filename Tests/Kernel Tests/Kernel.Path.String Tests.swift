...
extension Kernel.Path.String.Test.Unit {
    ...
    func singlePathNULAtStart() {
        ...
        expectThrows({ (error: E) in
            #expect(error == .interiorNUL(index: 0))
        }) { () throws(E) in
            _ = try Kernel.Path.scope("\0/tmp/file") { _ in () }
        }
    }
    ...
    func singlePathNULInMiddle() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("/tmp/\0file") { _ in () }
        }
    }
    ...
    func singlePathNULAtEnd() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("/tmp/file\0") { _ in () }
        }
    }
}
...
extension Kernel.Path.String.Test.Unit {
    ...
    func singlePathNULWrapped() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("\0/tmp/file") { (_: borrowing Kernel.Path) throws(Dummy) -> Void in
                ()  // never reached
            }
        }
    }
    ...
    func singlePathBodyErrorWrapped() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("/tmp/x") { (_: borrowing Kernel.Path) throws(Body) -> Void in
                throw .boom
            }
        }
    }
...
extension Kernel.Path.String.Test.Unit {
    ...
    func twoPathsNULInFirst() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("bad\0path", "/valid") { _, _ in () }
        }
    }
    ...
    func twoPathsNULInSecond() {
        typealias E = Kernel.Path.String.Conversion.Error
        expectThrows({ (error: E) in
            #expect(error == .interiorNUL(index: 1))
        }) { () throws(E) in
            _ = try Kernel.Path.scope("/valid", "bad\0path") { _, _ in () }
        }
    }
}
...
extension Kernel.Path.String.Test.Unit {
    ...
    func threePathsNULInFirst() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("bad\0", "/b", "/c") { _, _, _ in () }
        }
    }
    ...
    func threePathsNULInSecond() {
        ...
        expectThrows({ (error: E) in
            ...
            _ = try Kernel.Path.scope("/a", "bad\0", "/c") { _, _, _ in () }
        }
    }
    ...
    func threePathsNULInThird() {
        typealias E = Kernel.Path.String.Conversion.Error
        expectThrows({ (error: E) in
            #expect(error == .interiorNUL(index: 2))
        }) { () throws(E) in
            _ = try Kernel.Path.scope("/a", "/b", "bad\0") { _, _, _ in () }
        }
    }
}
