...
extension Kernel.Process.Group {
    ...
    public enum Target: Sendable, Equatable {
        ...
        case id(Kernel.Process.Group.ID)
    }
}
...
extension Kernel.Process.Group {
    ...
    /// - Throws: `Kernel.Process.Error.group` on failure.
    ///
    /// ## Mapping to setpgid
    ///
    ...
    /// try Kernel.Process.Group.set(.current, to: .id(pgid))
    ///
    /// // Move specific process to new group with that process as leader
    /// try Kernel.Process.Group.set(.id(childPid), to: .same)
    /// ```
    @inlinable
    ...
    /// let pgid = try Kernel.Process.Group.id(of: .current)
    ///
    /// // Get process group of another process
    /// let pgid = try Kernel.Process.Group.id(of: childPid)
    /// ```
    @inlinable
    public static func id(of pid: Kernel.Process.ID) throws(Kernel.Process.Error) -> ID {
