...
// ===----------------------------------------------------------------------===//
    ...
    extension Kernel.Process.Session {
        ...
        public typealias ID = Tagged<Kernel.Process.Session, pid_t>
    }
    ...
    extension Kernel.Process.Session {
        ...
        /// - Throws: `Kernel.Process.Error.session` on failure.
        ///
        /// ## Behavior
        ///
        ...
        /// case .child:
        ///     let sid = try Kernel.Process.Session.create()
        ...
        /// ```
        @inlinable
        public static func create() throws(Kernel.Process.Error) -> ID {
            let result = setsid()
            guard result != -1 else {
                throw .session(.captureErrno())
            }
            return ID(result)
        }
        ...
        /// let sid = try Kernel.Process.Session.id(of: .current)
        ///
        /// // Get session ID of another process
        /// let sid = try Kernel.Process.Session.id(of: somePid)
        /// ```
        @inlinable
