...
extension Kernel.Process.Wait {
    ...
    public enum Selector: Sendable, Equatable {
        ...
        case group(Kernel.Process.Group.ID)
...
extension Kernel.Process.Wait {
    ...
    public struct Result: Sendable, Equatable {
        ...
        public let status: Kernel.Process.Status

        @inlinable
        public init(pid: Kernel.Process.ID, status: Kernel.Process.Status) {
            self.pid = pid
            self.status = status
        }
    }
...
extension Kernel.Process.Wait {
    ...
    /// - Throws: `Kernel.Process.Error.wait` on failure.
    ///
    /// ## Common Errors
    ///
    ...
    ///
    /// // Non-blocking wait
    /// if let result = try Kernel.Process.Wait.wait(.any, options: .no.hang) {
    ...
    public static func wait(
        _ selector: Selector,
        options: Options = []
    ) throws(Kernel.Process.Error) -> Result? {
        let pid: pid_t = switch selector {
        case .any:
            -1
        case .process(let id):
            id.rawValue
        ...
        return Result(
            ...
            status: Kernel.Process.Status(rawValue: status)
        )
    }
}
