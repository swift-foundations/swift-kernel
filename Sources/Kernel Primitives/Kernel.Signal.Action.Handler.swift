...
// ===----------------------------------------------------------------------===//
    ...
    extension Kernel.Signal.Action {
        ...
        /// ```swift
        /// // Ignore SIGPIPE
        /// let config = Kernel.Signal.Action.Configuration(handler: .ignore)
        /// try Kernel.Signal.Action.set(signal: .pipe, config)
        ///
        /// // Custom handler
        /// let handler: @convention(c) (Int32) -> Void = { sig in
        ///     // Only async-signal-safe operations here
        /// }
        /// let config = Kernel.Signal.Action.Configuration(handler: .custom(handler))
        /// ```
    ...
    extension Kernel.Signal.Action.Handler {
