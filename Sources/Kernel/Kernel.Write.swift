...
extension Kernel.Write.Error: Equatable {
    public static func == (lhs: Self, rhs: Self) -> Bool {
        ...
        case let (.blocking(l), .blocking(r)): return l == r
        case let (.io(l), .io(r)): return l == r
        case let (.space(l), .space(r)): return l == r
        case let (.memory(l), .memory(r)): return l == r
...
extension Kernel.Write.Error {
    ...
    init(errno: Errno) {
        ...
        if let e = Kernel.Blocking.Error(errno: errno) { self = .blocking(e); return }
        if let e = Kernel.IO.Error(errno: errno) { self = .io(e); return }
        if let e = Kernel.Space.Error(errno: errno) { self = .space(e); return }
        if let e = Kernel.Memory.Error(errno: errno) { self = .memory(e); return }
