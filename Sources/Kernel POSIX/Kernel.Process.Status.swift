...
extension Kernel.Process.Status {
    ...
    public struct Exit: Sendable {
        ...
        init(_ status: Kernel.Process.Status) { self.status = status }
    ...
    public struct Terminating: Sendable {
        ...
        public var signal: Kernel.Signal.Number? {
            guard status.signaled else { return nil }
            return Kernel.Signal.Number(rawValue: swift_WTERMSIG(status.rawValue))
        }
    }
    ...
    public struct Stop: Sendable {
        ...
        public var signal: Kernel.Signal.Number? {
            ...
            return Kernel.Signal.Number(rawValue: swift_WSTOPSIG(status.rawValue))
        }
    }
...
extension Kernel.Process.Status {
    ...
    public enum Classification: Sendable, Equatable {
        ...
        case signaled(signal: Kernel.Signal.Number, Bool)

        /// Process stopped by signal.
        case stopped(signal: Kernel.Signal.Number)
